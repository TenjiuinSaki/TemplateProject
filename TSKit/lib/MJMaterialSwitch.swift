...
class MJMaterialSwitch: UIControl {
    ...
    convenience init(withSize size: MJMaterialSwitchSize, state: MJMaterialSwitchState) {
        ...
        case .normal:
            frame = CGRect(x: 0, y: 0, width: 53.em, height: 30.em)
            self.init(frame: frame)
        ...
        self.trackOnTintColor = R.color.main
        self.trackOffTintColor = ts.color.lightText

        self.thumbDisabledTintColor = UIColor.lightGray
        self.trackDisabledTintColor = UIColor.lightGray
...
//        self.isBounceEnabled = true
        ...
        self.switchThumb.layer.shadowOpacity = 0.3
        ...
        self.switchThumb.addTarget(self, action: #selector(self.onTouchDown(btn:withEvent:)), for: .touchDown)
        self.switchThumb.addTarget(self, action: #selector(onTouchUpOutsideOrCanceled(btn:withEvent:)), for: .touchUpOutside)
        self.switchThumb.addTarget(self, action: #selector(self.switchThumbTapped), for: .touchUpInside)
        self.switchThumb.addTarget(self, action: #selector(self.onTouchDragInside(btn:withEvent:)), for: .touchDragInside)
        self.switchThumb.addTarget(self, action: #selector(self.onTouchUpOutsideOrCanceled(btn:withEvent:)), for: .touchCancel)
